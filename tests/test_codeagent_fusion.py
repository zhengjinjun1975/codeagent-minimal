...
# ══════════ 1. 统一运行时：registry 原子全加载（对齐 registry.json 实际原子） ══════════
# 说明：registry.json 为唯一事实来源（原子数随注册动态）；随新原子注册自动对齐，
# 避免硬编码固定原子数导致本地与测试断言错位。
FUSED_ATOMS = {
    "arch-review", "atomicity-audit", "browser-smoke", "bug-deep", "code-deliver",
    "code-dispatch", "code-evolve", "code-fuzz", "code-implement", "code-memory",
