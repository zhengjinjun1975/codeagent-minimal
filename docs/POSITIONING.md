...
> **数据口径**：同类项目星标 / 许可 = **2026-09-23 经 GitHub API 实测**（`gh api repos/<owner>/<repo>`）；本仓数字取自 `registry.json`（39 原子 / 145 能力）与全库扫描（140 个 `.py` / 31,071 行 / **无 requirements.txt**）。
...
| **CodeAgent Minimal（本仓）** | — | Apache-2.0 | **质量治理内核** | **无（可选增强缺失即降级）** | **默认不出厂** | **✓（39 原子 / 145 能力）** |
...
2. **覆盖面 × 组装**：39 原子 / 145 能力，横跨代码审查、安全扫描、依赖 SCA、测试与变异、影响分析（方法级 / 依赖级）、死代码、架构审查、本体 / 领域审查、Git 只读工具、审批、沙箱、上下文压缩、模型降级；可任意组装成链（`guard` 安全质量链、`chain` think→gen→review→test→evolve）。
...
- 本仓：`registry.json`（39 原子 / 145 能力）；全库扫描 140 个 `.py` / 31,071 行 / 无 `requirements.txt`；测试数取自 `pytest` 实测。
