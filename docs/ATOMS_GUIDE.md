# CodeAgent Atoms 指南（核心 32 原子）

> 本指南逐个说明 32 个原子智能体：**能力（provides）/ 入参（inputs）/ 返回信封（`{ok, data}`）/ 示例代码**。
...
> 注册索引见 `registry.json`；运行时用 `python codeagent.py status --json` 查看 32 原子 ready/degraded/冲突状态。本指南深入讲解核心 32 原子；完整 32 原子清单见 [README](../README.md) 的「32 原子清单」。
...
32 原子经统一运行时按能力依赖（`depends_on`）做拓扑排序 + 冲突检测，可任意组装成链：
