...
复用（零改动核心）：review.py._list_code_atoms/_reuse_suggestion + atoms/ 复用库(自放,可空)
...
class CodeReuseAgent(AtomicAgent):
    ...
    version = "0.1.0"
    domain = "reuse"
    description = "代码复用检索: review.py 检索仓库内 atoms/ 可插拔复用库(随用户自放,默认空)"
    ...
    def _local(self, content=None, path=None, top_k=3):
        """本地检索: 复用 _reuse_suggestion(本地 atoms 库 → GitHub 远端降级)。
        path 为目录时聚合目录下 .py/.md 文件作为检索语料。"""
        src = content
        if not src and path:
            p = str(path)
            if os.path.isdir(p):
