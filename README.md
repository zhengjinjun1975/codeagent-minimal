...
> **v0.4.1 · 一体化完整版**：一个本地运行的代码智能体，`git clone` 即用，纯 Python 标准库、零第三方依赖、数据不出厂，Apache-2.0。
...
共 **34 个可独立运行、可任意组装的能力原子**，配合本地可视化 Lab 编排与浏览器前端。每个原子的能力 / 入参 / 示例见 [docs/ATOMS_GUIDE.md](docs/ATOMS_GUIDE.md)。

## 详细文档（都在这里，不放 README）

| 文档 | 说明 |
|------|------|
| [docs/ATOMS_GUIDE.md](docs/ATOMS_GUIDE.md) | 34 原子逐个指南：能力 / 入参 / 返回 / 示例 |
...
- `python codeagent.py status --json`：注册 **34 原子 / 118 能力**，`degraded=[]`、`conflicts=[]`（实测）。
